	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			for (int i = 0; i < consumerThreadCount; i++)
			{
				...
				// use the approximate num of primes to minimize list resizing
				List<ulong> primes = new List<ulong>((int)(approxNumPrimes / (uint)consumerThreadCount));

				//Add list to m_Primes, using the thread id as its key
			...
			// tell all consumers no more values will be produced
